
email_body_with_team_link = """
Bonjour,

Je vous confirme ma présence à cet entretien : {time}

Rejoignez la réunion sur votre ordinateur ou votre application mobile via le lien suivant:

{link}

Bien cordialement,
Houssem ADOUNI


------------------------------------------------------------------------------------------------------------------------------
Cet email est envoyée en automatique depuis mon application web www.houssem-adouni.com
si vous recevez cet email par erreur veuillez me le signaler /==> houssemadouni11@gmail.com /
------------------------------------------------------------------------------------------------------------------------------
"""

email_body_without_team_link = """
Bonjour,

Je vous confirme ma présence à cet entretien : {time}

Je vous enverrai le lien de la réunion Microsoft Teams 15 minutes avant le debut de la réunion.

Bien cordialement,
Houssem ADOUNI


------------------------------------------------------------------------------------------------------------------------------
Cet email est envoyée en automatique depuis mon application web www.houssem-adouni.com
Si vous recevez cet email par erreur veuillez me le signaler /==> houssemadouni11@gmail.com /
------------------------------------------------------------------------------------------------------------------------------
"""

email_body_phone="""
Bonjour,

Je vous confirme ma présence à cet entretien.

Voici mon numéro de télephone: +33758553568
N'oubliez pas de m'appeler sur ce numéro le {time}

Bien cordialement,
Houssem ADOUNI


------------------------------------------------------------------------------------------------------------------------------
Cet email est envoyée en automatique depuis mon application web www.houssem-adouni.com
Si vous recevez cet email par erreur veuillez me le signaler /==> houssemadouni11@gmail.com /
------------------------------------------------------------------------------------------------------------------------------
"""

email_body_skype="""
Bonjour,

Je vous confirme ma présence à cet entretien.

Voici mon identifiant Skype : {link}
N'oubliez pas de m'appeler sur le {time}

Bien cordialement,
Houssem ADOUNI


------------------------------------------------------------------------------------------------------------------------------
Cet email est envoyée en automatique depuis mon application web www.houssem-adouni.com
Si vous recevez cet email par erreur veuillez me le signaler /==> houssemadouni11@gmail.com /
------------------------------------------------------------------------------------------------------------------------------
"""

email_body_skype_without_link="""
Bonjour,

Je vous confirme ma présence à cet entretien : {time}

Je vous enverrai le lien de la réunion Skype 15 minutes avant le debut de la réunion.

Bien cordialement,
Houssem ADOUNI


------------------------------------------------------------------------------------------------------------------------------
Cet email est envoyée en automatique depuis mon application web www.houssem-adouni.com
Si vous recevez cet email par erreur veuillez me le signaler /==> houssemadouni11@gmail.com /
------------------------------------------------------------------------------------------------------------------------------
"""
